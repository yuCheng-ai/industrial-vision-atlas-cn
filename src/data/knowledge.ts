...
export const overviewGroups: KnowledgeGroup[] = [
  {
    title: '系统构成',
    intro: '工业视觉系统由成像、采集、处理、输出和控制集成组成。项目差异主要来自被测对象、运动方式、光学条件、接口带宽、算法类型和现场控制系统。',
    items: [
      { title: '照明 / 光源', body: '光源用于建立目标、背景、缺陷和边缘之间的对比关系。光源类型、角度、颜色、亮度和触发方式会影响图像稳定性。', source: 'Cognex Introduction to Machine Vision：hardware components / lighting' },
      { title: '镜头 / 光学', body: '镜头影响视野、工作距离、景深、畸变和测量一致性。精密测量需要关注畸变、放大倍率和物距变化。', source: 'Edmund Optics / Cognex 机器视觉光学资料' },
      ...
      { title: '输出 / 控制', body: '输出通常包括 OK/NG、坐标、尺寸数值、缺陷类别、读码结果、报警、剔除信号，或向 PLC、机器人、MES 传递数据。', source: 'Cognex / A3 machine vision introduction' },
    ],
  },
];
...
export const opticsGroups: KnowledgeGroup[] = [
  {
    title: '光源类型',
    intro: '光源按入射方向、均匀性、波长和几何结构区分。资料记录时应保留光源类型、角度、距离、亮度、颜色和触发方式。',
    items: [
      { title: '背光', body: '用于轮廓、孔位、边缘、尺寸测量。适合形成高对比轮廓，不用于观察表面纹理。', source: 'Cognex lighting guide / machine vision lighting practice' },
      { title: '环形光', body: '用于一般表面照明和有无检测。不同入射角度会改变反射、阴影和边缘表现。', source: 'Cognex lighting guide' },
      ...
      { title: '穹顶光 / 漫射光', body: '用于降低曲面、金属、塑料件表面的强反光，获得较均匀的外观图像。', source: 'Cognex lighting guide' },
      { title: '线光源', body: '常与线扫相机、激光轮廓或高速连续材料检测配合使用。', source: 'Line scan camera vendor application notes' },
    ],
  },
  {
    title: '镜头与成像参数',
    intro: '镜头参数围绕视野、工作距离、放大倍率、景深、畸变和分辨率组织。',
    items: [
      { title: '视野 FOV', body: '相机一次成像覆盖的实际物理范围。像素精度计算需要同时知道视野和传感器分辨率。', source: 'Edmund Optics imaging fundamentals' },
...
export const cameraGroups: KnowledgeGroup[] = [
  {
    ...
    items: [
      ...
      { title: '线扫相机', body: '逐行采集图像，适合卷材、薄膜、纸张、钢带、电池极片等连续运动材料。线扫系统通常需要运动控制和编码器同步。', source: 'Line scan camera vendor application notes' },
      { title: '3D 相机 / 轮廓仪', body: '获取高度、轮廓、点云或深度信息，适合胶路、焊缝、高度差、体积、形变等任务。', source: '3D vision camera vendor documentation' },
      { title: '智能相机', body: '将采集、处理和部分 I/O 集成在相机内部，适合结构较简单、部署空间有限或维护要求明确的单工位检测。', source: 'Cognex types of machine vision systems' },
    ],
  },
  {
    title: '相机性能参数',
    intro: '工业相机参数应以标准化指标和应用约束共同描述。EMVA 1288 用于机器视觉传感器和相机规格的测量与呈现。',
    items: [
...
export const interfaceGroups: KnowledgeGroup[] = [
  {
    title: '工业视觉接口标准',
    intro: '接口决定图像传输带宽、距离、同步方式、软件生态和采集卡需求。GenICam 在不同传输技术之上提供通用编程接口。',
    items: [
      ...
      { title: 'Camera Link / Camera Link HS', body: '高速视觉接口，常见于高带宽、低延迟、需要采集卡的工业系统。', source: 'A3 Vision Standards' },
...
export const calibrationGroups: KnowledgeGroup[] = [
  {
    title: '标定与测量',
    intro: '标定用于建立图像坐标、物理坐标、相机坐标、机器人坐标之间的关系。测量、定位和机器人引导场景需要单独记录标定方法、误差来源和复验条件。',
    items: [
      { title: '像素标定', body: '将图像像素与实际尺寸建立比例关系。简单平面测量可使用标尺或标定板，前提是工作平面、镜头和相机位置保持稳定。', source: 'OpenCV calib3d / 工业测量应用文档' },
      { title: '镜头畸变校正', body: '普通镜头存在径向和切向畸变。测量类项目需要评估畸变对边缘、孔位和轮廓尺寸的影响。', source: 'OpenCV camera calibration documentation' },
      { title: '相机外参', body: '外参描述相机相对工件、治具或机器人坐标系的位置和姿态。定位、抓取和装配场景需要记录外参来源。', source: 'OpenCV calib3d documentation' },
      { title: '手眼标定', body: '机器人视觉项目需要建立相机坐标与机器人坐标的转换关系。相机安装方式、机器人重复精度和标定姿态会影响结果。', source: 'OpenCV calib3d / robot hand-eye calibration references' },
      { title: '重复性与复验', body: '测量系统需要记录重复精度、温漂、机械振动、治具误差、光源稳定性和批次变化。', source: '工业测量系统分析 / 视觉项目验收资料' },
    ],
  },
];
...
export const sourceGroups: KnowledgeGroup[] = [
  {
    title: '标准与组织',
    intro: '标准组织和行业协会资料用于接口、相机指标和通用定义；厂商资料用于工程实现和应用说明；开源项目文档用于算法与软件生态。',
    items: [
      ...
      { title: 'OpenCV Documentation', body: '图像处理、标定、3D 重建、特征、目标检测和深度学习推理模块资料。', source: 'https://docs.opencv.org/' },
      { title: 'PaddleOCR', body: 'OCR、文本检测、文本识别、结构化文档解析和部署资料。', source: 'https://github.com/PaddlePaddle/PaddleOCR' },
      { title: 'Anomalib', body: '异常检测算法、实验管理和边缘推理资料。', source: 'https://github.com/open-edge-platform/anomalib' },
      { title: 'Ultralytics Docs', body: '目标检测、分割、姿态估计和模型部署资料。', source: 'https://docs.ultralytics.com/' },
    ],
  },
];
